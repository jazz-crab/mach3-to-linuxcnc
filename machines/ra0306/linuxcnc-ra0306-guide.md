...
**Шпиндель:** QW1.5/220/24K + частотник Lenze ESMD152X2SFA  
...
| Частотник | ESMD152X2SFA (Lenze SMD) |
...
> **ПК станка без интернета?** Пользуйся офлайн-набором из корня репозитория (`offline/`):
> `fetch-packages.py` скачивает на другой машине все `.deb` с зависимостями
> (LinuxCNC + RT-ядро + рабочий стол), а `setup-machine.sh` ставит их со флешки
> и настраивает систему — подробности в `offline/README.md`.
...
| Machine Units | **mm** |
| Driver Type | **Other** |
| Step Time | **12000** ns (с запасом; реальный минимум смотри в РЭ платы) |
| Step Space | **12000** ns |
...
> ⚠️ **Обороты шпинделя (важно).** Частотник Lenze ESMD152X2SFA ограничен
> параметром `C11` (макс. выходная частота). Заводское значение **240 Гц**
> даёт только **~14 400 об/мин** вместо паспортных 24 000 (шпиндель QW1.5 —
> 2-полюсный, 24 000 об/мин = 400 Гц).
>
> При пусконаладке: MDI `M3 S24000` → замерь реальные обороты → подними
> `C11` до **400 Гц** (если прошивка экземпляра позволяет — по мануалу
> `manuals/lenze-esmd152x2sfa.md`, §5), затем калибруй Speed2/PWM2.
> До этого калибруй под фактический потолок оборотов.
>
> **Клеммы частотника (Basic I/O):** задание оборотов 0–10 В — клемма **8**
> (общий аналоговый — клемма **7**); пуск/стоп — клемма **28** (HIGH = пуск);
> направление — цифровой вход **E2**; `C01 = 0` (задание от аналогового входа),
> `C34 = 0` (вход 0…10 В). Подробности — в `manuals/lenze-esmd152x2sfa.md`.
